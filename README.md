...
- 失败可重试；任务清单持久化到应用数据目录的 `install-tasks`，异常退出后可在新鲜安装页面选择未完成任务，手动继续或取消。正常关闭会先记录关闭标记并等待取消，不会下次自动安装。
- 恢复清单的 API Key / WebUI Key 使用 Electron safeStorage 加密（包括 `.bak`）；旧明文清单在成功读取后迁移。系统安全存储不可用时拒绝保存含密钥的新任务；Linux 不接受 `basic_text` 后端。实际安装配置文件仍按 MoFox 的格式写入密钥，应妥善保护实例目录。
...
- 从 GitHub Release 安装，支持镜像顺序轮询、SHA-256 校验、分块下载与安全解压。优先使用资产摘要，缺失时直连官方查找摘要或同版本校验文件；仍无法验证时弹出风险确认，默认取消，仅可明确允许本次下载。摘要冲突或下载校验不匹配时始终拒绝，不提供绕过选项。
...
- 界面语言（简体中文 / English）、默认安装目录；托盘、硬件加速及实例自动启动开关暂时隐藏，待完整实现后开放
