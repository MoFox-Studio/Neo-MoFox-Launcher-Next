...
// 主页面板：小部件的开关、排序与专属配置；全部修改整份提交到 home 设置字段。
...
const { moveWidget, updateWidget, resetHomeLayout } = useHomeWidgets();

const widgets = computed(() => settings.value.home.widgets);
...
const expandedId = ref<HomeWidgetId | null>(null);

function toggleExpanded(id: HomeWidgetId): void {
  expandedId.value = expandedId.value === id ? null : id;
}

function metaOf(id: HomeWidgetId) {
  return HOME_WIDGET_DEFINITIONS.find((definition) => definition.id === id)!;
}
...
function toggleQuickAction(id: QuickActionId): void {
  ...
  const next = actions.includes(id)
    ? actions.filter((entry) => entry !== id)
    : [...actions, id];
  ...
  <div class="settings-group">
    <section class="settings-group__card">
      <div class="settings-group__heading">
        ...
        <div>
          <h2>主页布局</h2>
          <p>开关小部件、调整顺序并配置各自的内容来源</p>
        </div>
      </div>
      <div class="home-list">
        <template v-for="(widget, index) in widgets" :key="widget.id">
          <div class="home-row settings-item">
            <span class="msr settings-item__icon">{{ metaOf(widget.id).icon }}</span>
            <div class="settings-item__body">
              <span class="settings-item__label">
                {{ metaOf(widget.id).title }}
                <span v-if="!widget.enabled" class="home-row__off-badge">已隐藏</span>
              </span>
              <span class="settings-item__desc">{{ metaOf(widget.id).description }}</span>
            </div>
            <button
              type="button"
            ...
            >
              <span class="msr" aria-hidden="true">arrow_upward</span>
            </button>
            <button
              type="button"
              class="home-row__icon-button state-layer"
              ...
              @click="moveWidget(widget.id, 1)"
            >
              <span class="msr" aria-hidden="true">arrow_downward</span>
            </button>
            <button
              type="button"
              class="home-row__icon-button state-layer"
              :title="expandedId === widget.id ? '收起配置' : '配置'"
              :aria-label="expandedId === widget.id ? '收起配置' : '配置'"
              :disabled="!widget.enabled"
              @click="toggleExpanded(widget.id)"
            >
              <span class="msr" aria-hidden="true">
                {{ expandedId === widget.id ? 'expand_less' : 'tune' }}
              </span>
            </button>
            <div
              class="md-switch"
              ...
              :aria-label="`显示${metaOf(widget.id).title}`"
              :class="{ 'md-switch--checked': widget.enabled }"
              @click="updateWidget(widget.id, { enabled: !widget.enabled })"
            >
          ...
          <!-- 专属配置区：按部件展开 -->
          <div v-if="widget.enabled && expandedId === widget.id" class="home-config">
            <!-- 时钟与日期 -->
            ...
            <template v-else-if="widget.id === 'docs'">
              ...
              <div class="home-config__row home-config__row--column">
                ...
                <p class="home-config__hint">
                  本地支持 .md / .markdown / .txt 文件；远程仅接受 HTTPS Markdown 链接；单个文档不超过 2 MB。
                </p>
    ...
    <section class="settings-group__card">
      <div class="settings-group__heading">
        ...
        <div>
          <h2>恢复默认</h2>
          <p>重置全部小部件的开关、顺序与配置</p>
        </div>
      </div>
      <div class="home-list">
        <div class="home-config__row">
          <div class="home-config__text">
            <span class="home-config__label">恢复默认布局</span>
            <span class="home-config__desc">隐藏的部件将重新显示，文档列表会被清空</span>
          </div>
          <button type="button" class="text-button state-layer" @click="resetHomeLayout">
            恢复默认
          </button>
        </div>
      </div>
...
<style scoped>
/* 部件列表容器：替代 settings-group__body 的首尾圆角语义，保持扁平行列表。 */
.home-list {
  display: grid;
  gap: 3px;
  padding: 0 10px 12px;
