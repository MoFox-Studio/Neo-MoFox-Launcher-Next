...
import type { VenvInfo } from '@shared/domain/venv';
...
async function installPackage(): Promise<void> {
  ...
  installError.value = '';
  try {
    const result = await mofoxApi.installVenvPackage(
      props.instance.id,
      name,
    ...
    emit('toast', `已安装 ${name}${installVersion.value ? ` ${installVersion.value}` : ''}`);
    await refresh();
  } catch (error) {
...
async function queryVersions(): Promise<void> {
  const name = installName.value.trim();
  if (!name) {
    installError.value = '请输入包名';
    return;
...
function closeInstallDialog(): void {
  if (installSubmitting.value) return;
  installOpen.value = false;
}
...
<template>
  <div class="venv-view">
    ...
    <!-- 安装依赖弹窗：输入包名 → 查询版本 → 选择版本安装 -->
    <BaseDialog
      :open="installOpen"
      title="安装依赖"
      :width="440"
      :dismissible="!installSubmitting"
      @close="closeInstallDialog"
    >
      <form class="install-form" @submit.prevent="installPackage">
        <div class="install-name-row">
          ...
          <button
            class="btn btn--tonal state-layer"
            type="button"
            :disabled="installQuerying || installSubmitting || !installName.trim()"
            @click="queryVersions"
          >
            <span class="msr btn__icon" aria-hidden="true">search</span>
            {{ installQuerying ? '查询中…' : '查询版本' }}
          </button>
        </div>
        <Transition name="field-error" mode="out-in">
          ...
          <p v-else key="hint" class="field__support">
            输入包名后点击「查询版本」，选择版本后安装；默认安装最新版。
          </p>
        </Transition>
        <div v-if="installVersions.length > 0" class="form-group">
          <label class="field field--grow">
            <select v-model="installVersion" class="field__select" :disabled="installSubmitting">
              <option value="">最新版本</option>
              <option v-for="v in installVersions" :key="v" :value="v">{{ v }}</option>
            </select>
      ...
      <template #actions>
        ...
        <button
          class="btn btn--filled state-layer"
          type="button"
          :disabled="installSubmitting || !installName.trim()"
          @click="installPackage"
        >
          <span v-if="!installSubmitting" class="msr btn__icon" aria-hidden="true">download</span>
