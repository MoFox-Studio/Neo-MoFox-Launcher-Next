import type { PlatformPaths } from '../../shared/domain/instance';

// ─── v1 → v2 仓库迁移 ────────────────────────────────────────────────────

/**
 * 把 v1 实例记录（仍含 `installPath` + `platformId` 单字段）升级为 v2 结构。
 *
 ...
 * @returns v2 结构的 `mofoxInstallDir` 与 `platforms` 字段；输入缺字段时给出安全默认值。
 */
export function upgradeInstancePathsToV2(record: {
  ...
  platformId?: string;
}): { mofoxInstallDir: string; platforms: PlatformPaths } {
  const mofoxInstallDir = record.mofoxInstallDir ?? record.installPath ?? '';
  const platformId = record.platformId ?? '';
  const platforms: PlatformPaths = { ...(record.platforms ?? {}) };
  if (platformId && !platforms[platformId] && mofoxInstallDir) {
    // v1 靠 dirname(installPath)/<platformId> 兄弟目录回退定位平台；v2 把它烘焙为显式路径。
    platforms[platformId] = joinPath(parentDir(mofoxInstallDir), platformId);
  }
